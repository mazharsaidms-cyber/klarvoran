...
document.querySelector("#report").addEventListener("click",()=>{
 ...
 const headings=Array.from(doc.querySelectorAll("h1,h2,h3")).filter(visible).map(el=>({text:el.textContent.trim(),width:el.clientWidth,scroll:el.scrollWidth})).filter(el=>el.scroll>el.width+1);
